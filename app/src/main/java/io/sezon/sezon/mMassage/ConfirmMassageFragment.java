...
public class ConfirmMassageFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        Realm realm = MangJekApplication.getInstance(activity).getRealmInstance();
        loginUser = realm.copyFromRealm(MangJekApplication.getInstance(activity).getLoginUser());

        massagePreference = activity.getMassagePreference();
        massageItem = activity.getMassageItem();
        mPayBalance.setText("Rp "+loginUser.getmPaySaldo());
        ...
        discountText.setText("Discount "+fitur.getDiskon()+" for using M-PAY");
...
//                }
    ...
    public void onResume() {
        super.onResume();
        User userLogin = MangJekApplication.getInstance(activity).getLoginUser();
    ...
    private void orderMPay() {
        ...
        dialog = ProgressDialog.show(getActivity(), "Mohon tunggu", "Mohon tunggu selagi request anda kami kirim...", true, false);
        ...
        service.requestTransaksiMMassage(param).enqueue(new Callback<RequestMassageResponseJson>() {
            ...
            public void onFailure(Call<RequestMassageResponseJson> call, Throwable t) {
                ...
                if(t instanceof DriverNotAvailable) {
                    Toast.makeText(activity, "Tidak ada driver tersedia.", Toast.LENGTH_SHORT).show();
                } else if(t instanceof RequestFailed) {
                    Toast.makeText(activity, "Request gagal. Silahkan coba request kembali.", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(activity, "Request gagal. Silahkan coba request kembali.", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void order() {
        ...
        if(massagePreference.getDurasiText().equalsIgnoreCase("60 minutes")) {
            ...
            durasi = 120;
        }else if(massagePreference.getDurasiText().equalsIgnoreCase("150 minutes")) {
            durasi = 150;
        }else if(massagePreference.getDurasiText().equalsIgnoreCase("180 minutes")) {
            durasi = 180;
        }
