...
import io.sezon.sezon.model.Pesanan;
...
/**
 * Created by bradhawk on 12/3/2016.
 */
...
//        diskonMpay = MangJekApplication.getInstance(this).getDiskonMpay();
        ...
        discountText.setText("Diskon "+fitur.getDiskon()+" Menggunakan PayPace");
    ...
    private void orderClick() {
        if(martName.getText().toString().trim().isEmpty()) {
            Toast.makeText(this, "Silahkan isi nama toko", Toast.LENGTH_SHORT).show();
        }

        if (martLatLng == null && martGetLocationText.getText().toString().trim().isEmpty()) {
            Toast.makeText(this, "Silahkan pilih lokasi toko", Toast.LENGTH_SHORT).show();
            return;
        }

        if (destinationLatLng == null && destinationGetLocationText.getText().toString().trim().isEmpty()) {
            Toast.makeText(this, "Silahkan pilih lokasi Pengantaran", Toast.LENGTH_SHORT).show();
            return;
        }

        if (estimatedCostEdit.getText().toString().trim().isEmpty()) {
            Toast.makeText(this, "Silahkan isi estimasi biaya", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        for (MartItem item : items) {
            if (item.getNamaProduk().trim().isEmpty()) {
                Toast.makeText(this, "Silahkan isi nama produk", Toast.LENGTH_SHORT).show();
                return;
            }
        }
...
//        }
    ...
    private void updateDistance(long distance) {
        ...
        String format = String.format(Locale.US, "Biaya (%.1f Km)", km);
        ...
        try {
            ...
            if (routes.size() > 0) {
                directionLine = gMap.addPolyline((new PolylineOptions())
                        ...
                        .color(ContextCompat.getColor(MartActivity.this, R.color.directionLine))
                        .width(5));
