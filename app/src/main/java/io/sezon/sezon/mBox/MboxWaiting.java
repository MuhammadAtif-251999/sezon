...
//                                }
                                ...
                                service.checkStatusTransaksi(param).enqueue(new Callback<CheckStatusTransaksiResponse>() {
                                        ...
                                        if(response.isSuccessful()) {
                                            ...
                                            if(checkStatus.isStatus()) {
                                                ...
                                                activity.runOnUiThread(new Runnable() {
                                                    @Override
                                                    public void run() {
                                                        Toast.makeText(MboxWaiting.this, "Sopir Anda terlihat sibuk! Silakan coba lagi.", Toast.LENGTH_LONG).show();
                                                    }
                                                });
                                    ...
                                    public void onFailure(Call<CheckStatusTransaksiResponse> call, Throwable t) {
                                        ...
                                        activity.runOnUiThread(new Runnable() {
                                            @Override
                                            public void run() {
                                                Toast.makeText(MboxWaiting.this, "Your drivers look busy! Please try again.", Toast.LENGTH_LONG).show();
                                            }
                                        });
    ...
    private void cancelOrder(){
        User loginUser = MangJekApplication.getInstance(MboxWaiting.this).getLoginUser();
            ...
            public void onResponse(Call<CancelBookResponseJson> call, Response<CancelBookResponseJson> response) {
                if (response.isSuccessful()) {
                    if (response.body().mesage.equals("order canceled")) {
                        Toast.makeText(MboxWaiting.this, "Pesanan Dibatalkan!", Toast.LENGTH_SHORT).show();
                        threadRun = false;
                        finish();
                    } else {
