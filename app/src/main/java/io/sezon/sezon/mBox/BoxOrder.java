...
import static io.sezon.sezon.mBox.BoxDetailOrder.ASALNAMA_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.ASALPHONE_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.ASAL_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.BARANG_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.CATATAN_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.DESTINATION_LOCATION;
import static io.sezon.sezon.mBox.BoxDetailOrder.ENDLATLONG_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.HARGA_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.JAM_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.JARAK_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.ORIGIN_LOCATION;
import static io.sezon.sezon.mBox.BoxDetailOrder.STRLATLONG_KEY;
import static io.sezon.sezon.mBox.BoxDetailOrder.TANGGAL_KEY;
import static io.sezon.sezon.mBox.MboxWaiting.DRIVER_LIST;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBoxAgreement.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                if (b) {
                    mBoxNext.setVisibility(VISIBLE);
                }
                else {
                    mBoxNext.setVisibility(GONE);
                }
            }
        });
        ...
        additionalShipperTextView.setText(shipperCount+" Additional Servis");
        ...
        loadingServiceCheckBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton compoundButton, boolean isChecked) {
                ...
                if(isChecked){
                    ...
                    mBoxAgreement.setVisibility(VISIBLE);

//                    shipperPrice.setVisibility(VISIBLE);
                    shipperCount = 1;
                }else{
                    mBoxAgreement.setChecked(false);
                    mBoxAgreement.setVisibility(GONE);
                    addShipper.setVisibility(GONE);
                    reduceShipper.setVisibility(GONE);
                    additionalShipperTextView.setVisibility(GONE);
//                    shipperPrice.setVisibility(GONE);
        ...
        service.getAdditionalMbox().enqueue(new Callback<GetAdditionalMboxResponseJson>() {
            ...
            public void onResponse(Call<GetAdditionalMboxResponseJson> call, Response<GetAdditionalMboxResponseJson> response) {
                if (response.isSuccessful()) {
                    ...
                    shipperPrice.setText("+USD "+additionalShipper+" per shipper");
...
//        if(requestCode>=DESTINATION_LOCATION_REQUEST_CODE){
////            mboxAdapter.getAdapterItem(0).
////            nextDestinasiList.get(resultCode-100).onActivityResult(requestCode,resultCode,data);
//            Log.e("RESULT", "OK");
////            mboxAdapter.getAdapterItem(requestCode-100).onActivityResult(requestCode,resultCode,data);
//        }
...
//                destinationMarker.add(marker);
        ...
        try {
            ...
            if (routes.size() > 0) {
                directionLine = gMap.addPolyline((new PolylineOptions())
                        ...
                        .color(ContextCompat.getColor(BoxOrder.this, R.color.directionLine))
                        .width(5));
    ...
    private void updateDistance(long distance) {
        ...
        if(distance > 1000){
            String jarak = String.format(Locale.US, "(%.1f Km)", km);
            //     priceTag.setText(jarak);
            long biayaTotal = (long) (selectedCargo.getHarga()*km);
            long biayaTotal1 = ((selectedCargo.getBiayaminimum())+biayaTotal);

            if(biayaTotal1 % 1 != 0)
                biayaTotal1 = (1 - (biayaTotal1 % 1)) + biayaTotal1;
            this.harga = biayaTotal1;

            if(biayaTotal1 < selectedCargo.getBiayaminimum()){
                this.harga = selectedCargo.getBiayaminimum();
                biayaTotal1 = selectedCargo.getBiayaminimum();

            }
            Log.e("jarak if",jarak+""+biayaTotal1+"");
            String formattedTotal = NumberFormat.getNumberInstance(Locale.US).format(biayaTotal1);
            String formattedText = String.format(Locale.US, "$ %s.00", formattedTotal);
            priceTag.setText(jarak+" "+formattedText);

        }else if(distance < 1000){

            Log.e("jarak else",mboxLocationOrigin.distance+"");
            long biayaTotal = ((selectedCargo.getBiayaminimum()));
            ...
            if(biayaTotal < selectedCargo.getBiayaminimum()){
                this.harga = selectedCargo.getBiayaminimum();
                biayaTotal = selectedCargo.getBiayaminimum();
            }

            String formattedTotal = NumberFormat.getNumberInstance(Locale.US).format(biayaTotal);
            String formattedText = String.format(Locale.US, "$ %s.00", formattedTotal);
            String jarak1 = String.format(Locale.US, "(%.1f Km)", km);
            priceTag.setText(jarak1+""+formattedText);
...
//        mboxAdapter.add(nextDestinasiList);
    ...
    private void removeItem() {
        ...
        if (mboxLocationListDest.size() > 1) mboxLocationListDest.remove(mboxLocationListDest.size() - 1);
